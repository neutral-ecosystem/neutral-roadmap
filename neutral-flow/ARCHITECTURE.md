...
A future Flow GUI is an authoring extension hosted by Neutral Editor. Flow
vocabulary schemas and conventions are exposed through the Neutral authoring
bridge as data-only descriptors, from which the Editor generates generic cards,
ports, and property controls. The Editor transcribes the visual model into
`.neu`, which still passes through `neutral-lang`; it does not emit a private
Flow format or bypass the language toolchain. Flow Core remains independently
headless and consumes Neutral IR, while Flow-owned system/provider mappers apply
CI/CD meaning. Neutral IR is the shared application boundary. Flow and Neux do
...
| **Neutral Editor Flow extension** | Visual authoring generated from Flow vocabulary schemas and conventions, transcribed to `.neu` | Must not bypass neutral-lang, execute CI/CD, or introduce a private Flow language/IR |
...
```text
text authoring ----------------\
Neutral Editor + Flow profile -> .neu -> neutral-lang -> Neutral IR -> neutral-flow -> CI/CD
                                                            \
                                                             -> neux -> GNU/Linux OS
```

There are exactly three domain applications in this model: neutral-lang,
neutral-flow, and neux. Neutral Editor is the shared authoring application; its
Flow extension is not a fourth domain language or runtime. Neutral IR is
generated by neutral-lang and consumed by the two domain applications; Flow and
Neux do not generate competing IRs.
